...
void mostrar_listado(int notas_alumnos[], bool criterio_b);
...
void mostrar_listado(int notas_alumnos[], bool criterio_b)
{
	...
	*/
	...
	case true:
		{
			int i;
			// Primero, hay que mostrar por pantalla los que esten aprobados primero
	...
	case false:
		{
			...
			*/
			...
			for (i = 0; i < ALUM; i ++)
			{auxiliar_notas[i] = -2; auxiliar_posiciones[i] = 0;}

			// Primero hay que copiar los datos al auxiliar_notas
			for(i = 0; i < ALUM; i++)
			...
			for(i = 0; i < ALUM; i ++)
			{
				if(auxiliar_notas[i] >= 0)
				{printf("Alumno %d: %d\n", auxiliar_posiciones[ALUM - 1 - i], auxiliar_notas[i]);}
			}
			break;
		}
	}
