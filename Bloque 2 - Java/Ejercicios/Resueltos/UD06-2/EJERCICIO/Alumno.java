...
public class Alumno
{
  private int notas[] = new int[5]; // Todo alumno está matriculado en cinco módulos.
  private String nombre;
  private int edad;
  static private float costeTitulo; // Para que sepan cuánto les va a costar el título.
  
  public Alumno()
  {
  }
  
  public Alumno(char pedir) // Para distinguirlo del constructor por defecto se le pone algo que lo distinga (el parámetro char, por ejemplo).
  {
    pedirTodosLosDatos();
  }
	  ...
	  do{
		...
		try
		{
		  ...
		  if (notas[pos] < 1 || notas[pos] > 10)
		    System.out.println("\n\tSe ha insertado una nota que no está entre 0 y 10. Por favor, intentalo de nuevo.");
		  else
		    entracorrec = 1;
		}
		catch (NumberFormatException e)
